...
open class CheckboxControl: UIControl {
    ...
    private var shouldCancelTouch = true
    var didTapHandler: ((Bool) -> Void)?
    ...
    var increasedTouchRadius: CGFloat = 0.1

    //By default it is true
    var useHapticFeedback: Bool = true

    private var startingTouchLocation: CGPoint?
    ...
    open override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        if let touch = touches.first {
            self.startingTouchLocation = touch.location(in: self)
            self.shouldCancelTouch = false
    ...
    open override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        if let touch = touches.first, let startingTouchLocation = self.startingTouchLocation {
            //fixes bug where 3d touch in Iphone 8 series was triggering this event
            if touch.location(in: self) != startingTouchLocation {
                self.shouldCancelTouch = true
            }
        }
    }
    //On touches ended,
    ...
    open override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        guard !shouldCancelTouch else { return }
